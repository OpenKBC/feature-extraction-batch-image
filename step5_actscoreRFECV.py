...
parser = argparse.ArgumentParser(prog='step5_actscoreRFECV.py')
# Input data
...
if __name__ == "__main__":
    ...
    inputFile = SharedFilePath+os.path.basename(step1Input).replace('.csv', '.step4.csv') # replace to step4 input
    ...
    outputFile = SharedFilePath+os.path.basename(step1Input).replace('.csv', '.step5.csv') # replace to step5 output
